...
function renderResult(result) {
  ...
  const details = [subject ? `Message: ${subject}` : "", scannedAt ? `Scanned: ${scannedAt}` : ""]
    .filter(Boolean)
    .join("\n");
...
function getFallbackMessage(verdict) {
  ...
  if (verdict === "scanning") return "Scanning current Gmail message...";
  if (verdict === "error") return "ThreatWatch could not complete the scan.";
  return "Open a Gmail message to see the latest result here.";
}
